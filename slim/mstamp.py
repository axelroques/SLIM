...
def mstamp(seq, sub_len, return_dimension=False):
    ...
    for i in range(sub_num):
        ...
        dist_profile[seq_sig <= _EPS] = np.inf  # Modified < into <=

        ########################
        ##### Modification #####
        ########################
        # We set the negative values (do not know why that
        # happens) to 0 to prevent sqrt(-float)
        dist_profile[np.where(dist_profile < 0)] = 0
...
def _mass_pre(seq, sub_len):
    ...
    ##### Modification #####
    ########################
    # We add the value of _EPS rather than 0 to seq_sig
    # to prevent dividing by zero
    seq_sig[np.where(seq_sig == 0)] = _EPS
